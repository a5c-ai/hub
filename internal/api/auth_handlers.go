...
func (h *AuthHandlers) OAuthCallback(c *gin.Context) {
	...
	response, err := h.oauthService.HandleCallback(c.Request.Context(), provider, code, state)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *AuthHandlers) SetupMFA(c *gin.Context) {
	...
	response, err := h.mfaService.SetupTOTP(userID.(uuid.UUID), "Hub", username.(string))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
