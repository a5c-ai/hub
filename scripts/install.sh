...
if [[ -d "frontend" && -f "frontend/package.json" ]]; then
    ...
    for i in {1..3}; do
        ...
        if [[ -f "package-lock.json" ]]; then
            if timeout 30m npm ci --prefer-offline --no-audit --no-fund --progress=false --legacy-peer-deps --force; then
                echo "✅ Frontend dependencies installed successfully"
                break
            fi
        else
            if timeout 30m npm install --prefer-offline --no-audit --no-fund --progress=false --legacy-peer-deps --force; then
                echo "✅ Frontend dependencies installed successfully"
                break
            fi
        fi
