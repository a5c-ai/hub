        ...
        if [[ -n "$AUTH_GITHUB_CLIENT_ID" && -n "$AUTH_GITHUB_CLIENT_SECRET" ]]; then
            ...
            if [[ "$DRY_RUN" == "true" ]]; then
                kubectl patch secret hub-secrets -n "$NS" --type merge \
                  -p "{\"data\":{\"github-oauth-client-id\":\"$(echo -n \"$AUTH_GITHUB_CLIENT_ID\" | base64)\",\"github-oauth-client-secret\":\"$(echo -n \"$AUTH_GITHUB_CLIENT_SECRET\" | base64)\"}}"
            else
                kubectl patch secret hub-secrets -n "$NS" --type merge \
                  -p "{\"data\":{\"github-oauth-client-id\":\"$(echo -n \"$AUTH_GITHUB_CLIENT_ID\" | base64)\",\"github-oauth-client-secret\":\"$(echo -n \"$AUTH_GITHUB_CLIENT_SECRET\" | base64)\"}}"
            fi
        else
            warn "AUTH_GITHUB_CLIENT_ID or AUTH_GITHUB_CLIENT_SECRET not set; skipping GitHub OAuth secret sync"
        fi
