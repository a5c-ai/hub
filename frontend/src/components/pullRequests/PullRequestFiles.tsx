...
export function PullRequestFiles({ files }: PullRequestFilesProps) {
    ...
    return lines.map((line, index) => {
      ...
      return (
        <div key={index} className={className}>
          <span className="select-none text-muted-foreground mr-2 w-8 inline-block text-right">
            {index + 1}
          </span>
          {line}
        </div>
  ...
  if (files.length === 0) {
    return (
      <Card className="p-8 text-center">
        <p className="text-muted-foreground">No files changed in this pull request</p>
      </Card>
    )
  }
