...
describe('SSHKeyManagement', () => {
  ...
  test('renders empty state when no SSH keys', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('You haven\'t added any SSH keys yet. Add one to securely access your repositories.')).toBeInTheDocument();
    });
  });
  ...
  test('validates SSH key format', async () => {
    ...
    const submitButton = screen.getAllByRole('button', { name: /Add SSH Key/ }).find(btn => !btn.disabled);
    ...
    fireEvent.change(keyInput, { 
      target: { value: 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG4rT3vTt99Ox5kndS4HmgTrKBT8SKzhK4rhGkEVGlCI test@example.com' } 
    });
  ...
  test('creates new SSH key successfully', async () => {
    ...
    await waitFor(() => {
      expect(sshKeyApi.createSSHKey).toHaveBeenCalledWith({
        title: 'New key',
        key_data: 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG4rT3vTt99Ox5kndS4HmgTrKBT8SKzhK4rhGkEVGlCI test@example.com',
      });
    });
  });
